...
public sealed class CashOperationModel
{
    ...
    {
        CashOperationKind.Deposit or CashOperationKind.OpeningBalance or CashOperationKind.ShiftOpen => new SolidColorBrush(Color.FromRgb(5, 150, 105)),
        CashOperationKind.Withdrawal or CashOperationKind.ShiftClose => new SolidColorBrush(Color.FromRgb(185, 28, 28)),
    ...
    public Brush AmountBrush => Kind is CashOperationKind.Withdrawal or CashOperationKind.ShiftClose
    ...
    public static CashOperationKind ResolveKind(string? type)
    {
        ...
        if (t.Contains("Внес", StringComparison.OrdinalIgnoreCase) || t.Contains("Начальн", StringComparison.OrdinalIgnoreCase))
            return t.Contains("Начальн", StringComparison.OrdinalIgnoreCase)
                ? CashOperationKind.OpeningBalance
                : CashOperationKind.Deposit;
        if (t.Contains("Изъят", StringComparison.OrdinalIgnoreCase))
            return CashOperationKind.Withdrawal;
        if (t.Contains("Открыт", StringComparison.OrdinalIgnoreCase))
            return CashOperationKind.ShiftOpen;
        if (t.Contains("Закрыт", StringComparison.OrdinalIgnoreCase))
            return CashOperationKind.ShiftClose;
        return CashOperationKind.Other;
    }
}
